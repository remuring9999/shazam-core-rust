...
export class FrequencyPeak {
  ...
  getAmplitudePcm() {
    return (
      Math.sqrt(
        (Math.exp((this.peakMagnitude - 6144) / 1477.3) * (1 << 17)) / 2
      ) / 1024
    );
  }
...
const padTo32 = (data: Uint8Array) =>
  new Uint8Array([...data, ...Array(4 - data.length).fill(0)]);
const readInt32 = (data: Uint8Array) => new Int32Array(data)[0];
const writeUint32 = (e: number) => [
  e & 0xff,
  (e >> 8) & 0xff,
  (e >> 16) & 0xff,
  (e >> 24) & 0xff,
...
export class DecodedMessage {
  ...
  static decodeFromBinary(bytes: Uint8Array) {
    ...
    self.sampleRateHz = parseInt(
      SampleRate[header.shiftedSampleRateId >> 27].substring(1)
    );
  ...
  static decodeFromUri(uri: string) {
    ...
    return this.decodeFromBinary(
      Buffer.from(uri.replace(DATA_URI_PREFIX, ""), "base64")
    );
  }
  ...
  encodeToBinary() {
    const header: RawSignatureHeader = {
      ...
      magic2: 0x94119c00,
      shiftedSampleRateId:
        (SampleRate[`_${this.sampleRateHz}` as any] as unknown as number) << 27,
    ...
    for (const x of Object.entries(this.frequencyBandToSoundPeaks)
      ...
      for (const frequencyPeak of frequencyPeaks) {
        ...
        peaksBuffer.push(
          ...writeInt16(frequencyPeak.correctedPeakFrequencyBin - 1)
        );
        fftPassNumber = frequencyPeak.fftPassNumber;
      }
